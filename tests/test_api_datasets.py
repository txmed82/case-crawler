...
def test_generate_dataset_api_accepts_external_clinical_text_backend(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)
    ...
    response = client.post(
        ...
        json={
            ...
            "clinical_text_command": ["hf-note-sample", "--profile", "synthetic-notes"],
        },
    )
...
def test_dataset_api_generates_release_package_with_fixture_references(
    ...
    assert (
        summary["objective_coverage"]["criteria"]["messy_clinical_text"]["satisfied"]
        is True
    )
    assert (
...
def test_dataset_api_imports_local_reference_dataset(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    client = TestClient(app)
    local_path = tmp_path / "local-reference.json"
                ...
                "rows": [
                    {
                        "subject_id": "local-1",
                        "clinical_note": (
                            "Progress Note: 57-year-old female with COPD."
                        ),
                        "prompt": "Extract diagnosis.",
                        "completion": "COPD.",
                        "task_name": "extraction",
    ...
    response = client.post(
        ...
        json={
            "path": str(local_path),
            "reference_key": "local-copd-notes",
            "dataset_id": "ds-local-reference",
            "license": "cc-by-4.0",
            "note_field": "clinical_note",
            "question_field": "prompt",
            "answer_field": "completion",
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["dataset_id"] == "ds-local-reference"
    assert body["reference_key"] == "local-copd-notes"
    assert body["repo_id"] == str(local_path)
