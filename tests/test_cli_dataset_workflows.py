...
def test_dataset_cli_lists_clinical_text_model_policy_metadata(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()

    result = runner.invoke(cli, ["clinical-text-models"])

    assert result.exit_code == 0
    assert "medgemma_4b_it: adapter=external_command" in result.output
    assert "model_id=google/medgemma-4b-it" in result.output
    assert "license=health-ai-developer-foundations" in result.output
    assert "use_policy=health_ai_terms_review_outputs_before_release" in result.output
    assert "command=hf-note-sample --model google/medgemma-4b-it" in result.output
    assert "stdout=ClinicalDocument[]" in result.output
    assert "meditron_7b: adapter=external_command" in result.output
    assert "model_id=epfl-llm/meditron-7b" in result.output
    assert "license=llama2" in result.output
...
def test_dataset_cli_generates_release_package_with_fixture_references(
    ...
    assert (
        release_summary["objective_coverage"]["criteria"]["physician_notes"][
            "satisfied"
        ]
        is True
    )
    assert (
        release_summary["objective_coverage"]["criteria"]["nursing_notes"][
            "satisfied"
        ]
        is True
    )
...
def test_dataset_cli_requires_release_summary_for_release_verification(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)
    ...
    assert generated.exit_code == 0, generated.output
    summary_path = tmp_path / "release-package" / "release_package_summary.json"
    summary_path.unlink()
    manifest_path = tmp_path / "release-package" / "manifest.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["audit_artifacts"].pop("release_package_summary.json")
    manifest["files"].pop("release_package_summary.json")
    manifest_path.write_text(json.dumps(manifest))
    ...
    assert release_verified.exit_code != 0
    assert "missing release audit artifact" in release_verified.output
    assert "release_package_summary.json" in release_verified.output
...
def test_dataset_cli_blocks_export_until_required_human_review(tmp_path, monkeypatch):
    ...
    assert json.loads(summary_before.output)["missing_required_review"] == 1
    assert marked.exit_code == 0
    assert json.loads(summary_after.output)["approved"] == 1
    assert json.loads(summary_after.output)["missing_required_review"] == 0
    assert exported.exit_code == 0
    assert (tmp_path / "synthetic.jsonl").exists()
...
def test_dataset_cli_passes_external_clinical_text_options(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    captured = []
    ...
    result = runner.invoke(
        ...
        [
            ...
            "hf-note-sample,--profile,synthetic-notes",
        ],
    )

    assert result.exit_code == 0
    assert captured[0].modalities == [Modality.CLINICAL_TEXT]
    assert captured[0].clinical_text_backend == "external"
    assert captured[0].clinical_text_command == [
        "hf-note-sample",
        "--profile",
        "synthetic-notes",
    ]
...
def test_dataset_cli_passes_clinical_text_model_profile(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    captured = []
...
def test_dataset_cli_passes_clinical_text_noise_profile(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    captured = []
    ...
    assert result.exit_code == 0
    assert captured[0].clinical_text_noise_profile == "heavy"
